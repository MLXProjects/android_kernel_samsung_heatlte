...
static phys_addr_t mdm_trace_len = SDRAM_STM_TRACE_BUFFER_END_ADDR -
				SDRAM_STM_TRACE_BUFFER_START_ADDR+1;
...
static void mdm_mbre_env_write(const char *key, const char *value)
{
	...
	rmc_loader_phys_write(SDRAM_MODEM_START_ADDR + MODEM_MBRE_START_OFFSET +
...
static void populate_MBRE(void)
{
	...
	mdm_nc_size = mdm_mpu_safe(
		(SDRAM_SDTOC_END_ADDR + 1) -
		(SDRAM_DIAMOND_END_ADDR + 1));

	MBRE_add_address("SDRAM_MDM_NONCACHE",
		SDRAM_DIAMOND_END_ADDR + 1,
		SDRAM_DIAMOND_END_ADDR + mdm_nc_size);

#ifdef SDRAM_DIAMOND_START_ADDR
	/* Add DIAMOND lib info to MBRE */
	MBRE_add_address("SDRAM_DMND",
...
static loff_t rmc_loader_llseek(struct file *file, loff_t offset, int origin)
{
	...
	case 0: /* SEEK_SET */
		...
		case MDM_SEEK_MBRE_ENV:
			 ...
			 *
			 * Assume DEVICE access, not cached */
			remapped_phys_addr = ioremap(SDRAM_MODEM_START_ADDR, 4*4);
			if (remapped_phys_addr) {
...
//	printk(KERN_ALERT "rmc_loader_write(len=%d)\n", (int)len);
	...
	if (MDM_SEEK_MBRE_ENV == mdm_seek_status) {
		...
		if (copy_from_user(toc_buffer, buf, len)) {
			...
			if (rmc_loader_phys_write(SDRAM_MODEM_START_ADDR + MODEM_MBRE_START_OFFSET + mdm_mbre_env_offset, toc_buffer, len) != 0) {
                                printk(KERN_ALERT "rmc_loader_write call to _phys_write fails for MBRE ENV\n");
                                goto fault_reset;
                        }
			*ppos += len;
			...
			*ppos += len;
			if ((toc_ptr - toc_buffer) >= TOC_ENTRY_SIZE) {
				...
				if (toc[toc_index].load_address==0x40000000 &&
					SDRAM_MODEM_START_ADDR!=0x40000000) {
					pr_alert("TOC - Overwriting modem start addr.\n");
					toc[toc_index].load_address=SDRAM_MODEM_START_ADDR;
				}
...
static int rmc_loader_open(struct inode *inode, struct file *file)
{
	...
	if ((file->f_flags & O_ACCMODE) == O_WRONLY) {
		...
		pr_alert("T004 OCPBRGWIN1 0x%x\n",
		       (SDRAM_MODEM_START_ADDR & MODEM_WINDOW_MASK));
		__raw_writel(SDRAM_MODEM_START_ADDR & MODEM_WINDOW_MASK,
...
bool mdm_sdram_trace_small(void)
{
	return sdram_trace_small;
}
...
static int __init rmc_loader_init(void)
{
	...
	mdm_trace_start = APE2MDM(SDRAM_STM_TRACE_BUFFER_START_ADDR);
	mdm_to_ape_addr = SDRAM_MODEM_START_ADDR - MODEM_BASE_ADDRESS;
