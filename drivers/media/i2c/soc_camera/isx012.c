...
static DEVICE_ATTR(rear_flash, S_IWUSR|S_IWGRP|S_IWOTH, NULL, maincamflash_ISX012_store);
static DEVICE_ATTR(rear_vendorid, 0644, mainvendorid_ISX012_show, NULL);
...
static int _ISX012_i2c_send(struct i2c_client *client, const u8 * data, int len)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to send %d bytes [errno=%d]",
			len, ret);
	} else if (ret != len) {
		printk(
		KERN_ERR "Failed to send exactly %d bytes (send %d)",
		len, ret);
		ret = -EIO;
	} else {
		ret = 0;
