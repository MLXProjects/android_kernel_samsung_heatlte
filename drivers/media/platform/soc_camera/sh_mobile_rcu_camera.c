...
static const struct v4l2_ctrl_config rcu_custom_cam_capture_param_cfg = {
	.ops    = &sh_mobile_rcu_ctrl_ops,
	.id     = V4L2_CID_PARAMETERS,
	.type   = V4L2_CTRL_TYPE_INTEGER,
	.name   = "RCU Set Parameters",
	.min    = INT_MIN,
	.max    = INT_MAX,
	.step   = 1,
	.def    = RCU_CUSTOM_CMD_DEFAULT_VALUE,
	...
	if (ctrl->val != RCU_CUSTOM_CMD_DEFAULT_VALUE) {
		...
		case V4L2_CID_SET_RESET:
			if (ctrl->val)
				...
				power = 1;
			if (sdesc->power) {
				ret = sdesc->power(&client->dev, power);
				if (ret < 0) {
...
static int sh_mobile_rcu_g_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	ret = v4l2_subdev_call(sd, core, g_ctrl, &v4l_ctrl);
	if (!ret)
		ctrl->val = v4l_ctrl.value;
		...
		unsigned int idx, struct soc_camera_format_xlate *xlate)
{
	...
	v4l2_ctrl_new_custom(&icd->ctrl_handler,
	 ...
	 v4l2_ctrl_new_custom(&icd->ctrl_handler,
				&rcu_custom_cam_capture_param_cfg, NULL);
