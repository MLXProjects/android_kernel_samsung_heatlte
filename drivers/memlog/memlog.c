...
#define MEMLOG_SIZE (SDRAM_MEMLOG_END_ADDRESS - SDRAM_MEMLOG_START_ADDRESS + 1)
...
{
	return (phys_addr_t)SDRAM_MEMLOG_START_ADDRESS + (logdata - pm_area);
}
...
static inline void memory_log_proc(const char *name, unsigned long pid)
{
	...
	strncpy(log.data, name, TASK_COMM_LEN);
...
static int __init init_memlog(void)
{
	int ret = 0;
	int cpu;
	unsigned int memlog_size = MEMLOG_SIZE;
	unsigned int per_cpu_buffers_min_size;
	unsigned int per_cpu_buffers_multiply;
	char *memlog_data;

	if (SDRAM_MEMLOG_START_ADDRESS > memblock_end_of_DRAM())
		logdata = (char __force *)ioremap_nocache(
				SDRAM_MEMLOG_START_ADDRESS, MEMLOG_SIZE);
	else
		logdata = (char __force *)ioremap_wc(SDRAM_MEMLOG_START_ADDRESS,
				MEMLOG_SIZE);
	if (!logdata) {
	...
	register_ramdump_split("memlog", SDRAM_MEMLOG_START_ADDRESS,
			SDRAM_MEMLOG_START_ADDRESS+MEMLOG_SIZE-1);
