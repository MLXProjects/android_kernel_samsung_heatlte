...
{
	pinctrl_pm_select_default_state(pd->dev);
...
static void deactivate_ch(struct sh_mobile_i2c_data *pd)
{
	...
	pinctrl_pm_select_idle_state(pd->dev);
}
...
static int sh_mobile_i2c_probe(struct platform_device *dev)
{
	...
	pd->timeout = 5000; /* in msecs */
	if (pdata) {
		if (pdata->bus_speed)
			pd->bus_speed = pdata->bus_speed;
		if (pdata->timeout_ms)
			pd->timeout = pdata->timeout_ms;
	} else {
		u32 bus_speed = 0;
                u32 xfer_timeout = 0;
		if (!of_property_read_u32(np, "clock-frequency", &bus_speed))
			pd->bus_speed = bus_speed;
                if (!of_property_read_u32(np,  "renesas,transfer-timeout-ms", &xfer_timeout)) {
                        pd->timeout = xfer_timeout;
		}
	}
