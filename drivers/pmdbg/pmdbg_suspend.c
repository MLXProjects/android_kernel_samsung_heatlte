...
static int pmdbg_dump_active_clks;
...
DECLARE_CMD(enable_clk_dump, enable_clk_dump_suspend_cmd);
DECLARE_CMD(disable_clk_dump, disable_clk_dump_suspend_cmd);
...
static int enable_clk_dump_suspend_cmd(char *para, int size)
{
	FUNC_MSG_IN;
	pmdbg_dump_active_clks = 1;
	FUNC_MSG_RET(0);

}

static int disable_clk_dump_suspend_cmd(char *para, int size)
{

	FUNC_MSG_IN;
	pmdbg_dump_active_clks = 0;
	FUNC_MSG_RET(0);
}

int pmdbg_get_clk_dump_suspend(void)
{
	return pmdbg_dump_active_clks;
}
...
static int suspend_init(void)
{
	...
	ADD_CMD(suspend, enable_clk_dump);
	ADD_CMD(suspend, disable_clk_dump);
	pmdbg_enable_dump_suspend = 0;
...
static void suspend_exit(void)
{
	...
	DEL_CMD(suspend, enable_clk_dump);
	DEL_CMD(suspend, disable_clk_dump);
