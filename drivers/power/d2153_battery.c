...
static u16 adc_weight_section_discharge_rlt_1c[] = {385, 523, 596, 653, 193,  44,  15,  14,  19,  37,  38,  64, 100, 127, 1120};
static u16 adc_weight_section_discharge_lt_1c[]  = {385, 523, 596, 653, 193,  44,  15,  14,  19,  37,  38,  64, 100, 127, 1120};
static u16 adc_weight_section_discharge_lmt_1c[] = {385, 523, 596, 653, 193,  44,  15,  14,  19,  37,  38,  64, 100, 127, 1120};
static u16 adc_weight_section_discharge_llt_1c[] = {385, 523, 596, 653, 193,  44,  15,  14,  19,  37,  38,  64, 100, 127, 1120};
...
static u16 adc_weight_section_discharge_rlt_2c[] = {215, 413, 494, 306,  98,  49,  18,  17,  22,  46,  44,  67, 103, 128, 550};
static u16 adc_weight_section_discharge_lt_2c[]  = {215, 413, 494, 306,  98,  49,  18,  17,  22,  46,  44,  67, 103, 128, 550};
static u16 adc_weight_section_discharge_lmt_2c[] = {215, 413, 494, 306,  98,  49,  18,  17,  22,  46,  44,  67, 103, 128, 550};
static u16 adc_weight_section_discharge_llt_2c[] = {215, 413, 494, 306,  98,  49,  18,  17,  22,  46,  44,  67, 103, 128, 550};
#endif
...
#define CONFIG_D2153_BATTERY_FORCE_FULL


#ifdef CONFIG_D2153_BATTERY_FORCE_FULL
static int d2153_set_soc_forcefull(struct d2153_battery *pbat)
{
	int ret = 0;
...
static int d2153_get_soc(struct d2153_battery *pbat)
{
	...
	if(ps_bat == NULL) {
		pr_batt(ERROR, "%s. Power supply for battery is null\n", __func__);
		return -EINVAL;
	}

	ps_bat->get_property(ps_bat, POWER_SUPPLY_PROP_STATUS, &value);
	pr_batt(VERBOSE,"%s. POWER_SUPPLY_PROP_STATUS = %d\n", __func__, value.intval);
	if(value.intval == POWER_SUPPLY_STATUS_FULL 
		&& soc < FULL_CAPACITY && pbat_data->is_charging)
	{
		pr_batt(ERROR,"%s. Exception occured. soc = %d, SOC will be set to FULL.\n",
					__func__, soc);
		d2153_set_soc_forcefull(pbat);
		soc = FULL_CAPACITY;
	}
#endif
