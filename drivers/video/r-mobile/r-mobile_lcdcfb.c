...
static int sh_mobile_lcdc_suspend(void);
static int sh_mobile_lcdc_resume(void);
...
static void sh_mobile_fb_vsync_acknowledge(struct sh_mobile_lcdc_priv *priv)
{
...
static int sh_mobile_fb_blank(int blank, struct fb_info *info)
{
	if ((blank == FB_BLANK_NORMAL) || (blank == FB_BLANK_POWERDOWN))
		return sh_mobile_lcdc_suspend();
	else if (blank == FB_BLANK_UNBLANK)
		return sh_mobile_lcdc_resume();
	else
		return -EINVAL;
}
	...
	for (i = 0; i < ARRAY_SIZE(pdata->ch); i++) {
		...
		case LCDC_CHAN_MAINLCD:
			lcd_ext_param[i].panel_func
				= pdata->panel_func(RT_DISPLAY_LCD1);
		...
		case LCDC_CHAN_SUBLCD:
			lcd_ext_param[i].panel_func
				= pdata->panel_func(RT_DISPLAY_LCD2);
			if (!lcd_ext_param[i].panel_func.panel_info) {
				j++;
				break;
			}
...
/* onscreen buffer 2 */
	...
	sh_lcd_vsync.wq_vsync_off = create_singlethread_workqueue("vsync_cpuidle_wq");
...
static void sh_mobile_lcdc_shutdown(struct platform_device *pdev)
{
	
	unsigned int lcd_num;
	for (lcd_num = 0; lcd_num < CHAN_NUM; lcd_num++) {
		if (lcd_ext_param[lcd_num].panel_func.panel_shutdown) {
				lcd_ext_param[lcd_num].
					panel_func.panel_shutdown();
				printk(KERN_ALERT "sh_mobile_lcdc_shutdown()\n");
		}
	}
...
static struct platform_driver sh_mobile_lcdc_driver = {
	...
	.shutdown   = sh_mobile_lcdc_shutdown,
};
