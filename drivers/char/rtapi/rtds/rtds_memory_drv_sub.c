...
/* Global variables for mmu_flush debug */
...
static atomic_t reg_mem;
static struct reg_lmk rt_reg_lmk;
struct reg_show_mem rt_reg_show_mem;

/**** prototype ****/
static unsigned long rtds_memory_tablewalk(unsigned long virt_addr);

static int rt_lmk_cbk(struct reg_lmk *reg_lmk, struct lmk_op *op)
{
	return atomic_read(&reg_mem);
}

static void rt_show_mem_cbk(struct reg_show_mem *reg_show_mem)
{
	pr_info("rt total:%d\n", atomic_read(&reg_mem));
}

static int __init rt_sub_init(void)
{
	rt_reg_lmk.cbk = rt_lmk_cbk;
	register_lmk(&rt_reg_lmk);

	rt_reg_show_mem.cbk = rt_show_mem_cbk;
	register_show_mem(&rt_reg_show_mem);

	return 0;
}
module_init(rt_sub_init);
/*****************************************************************************
...
#endif
	default:
		MSG_ERROR("[RTDSK]   | Unknown mpro event(0x%lx)\n",
				mem_table->event);
		break;
	}
...
)
{
	...
	unsigned long	phy_pte, phy_pte_hw;
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 4, 0)
	...
	dmac_map_area(pgd, sizeof(pgd_t), DMA_TO_DEVICE);
	dmac_map_area(pte, sizeof(pte_t), DMA_TO_DEVICE);
	dmac_map_area(pte_hw, sizeof(pte_t), DMA_TO_DEVICE);

	pte_unmap(pte);

	phy_pgd = virt_to_phys(pgd);
	phy_pte = (pmd_val(*(pmd)) & PHYS_MASK & (s32)PAGE_MASK)
		+ pte_index(address) * sizeof(pte_t);
	phy_pte_hw = phy_pte + PTE_HWTABLE_OFF;

	outer_clean_range(phy_pgd, phy_pgd + sizeof(pgd_t));
	outer_clean_range(phy_pte, phy_pte + sizeof(pte_t));
	outer_clean_range(phy_pte_hw, phy_pte_hw + sizeof(pte_t));
...
)
{
	...
	if (NULL == create_queue) {
		...
		create_queue->tgid	= current->tgid;
...
)
{
	...
	list_for_each_entry(mem_table, &g_rtds_memory_list_shared_mem,
		list_head) {
		MSG_ERROR("[RTDSK]   |[%d]: rt[%#x %#x] sz[%#x] tgid[%d]\n",
				i, (u32)mem_table->rt_wb_addr,
				(u32)mem_table->rt_nc_addr,
				(u32)mem_table->memory_size,
				(u32)mem_table->tgid);
		i++;
	...
	up(&g_rtds_memory_shared_mem);
	MSG_ERROR("[RTDSK]   |<---Mpro list dump end---\n");
	MSG_MED("[RTDSK]OUT|[%s]\n", __func__);
	return;
}
	...
	if (SMAP_OK == ret) {
		...
		if (RTDS_MEM_ATTR_MEDIA == cma_list->mem_attr) {
			...
			if (SEC_HAL_CMN_RES_OK != sec_ret) {
				...
				ret = SMAP_NG;
				/* rt_cma_drv_free is not called as it is not
				 * safe to free protected memory to linux
				 */
				kfree(cma_list);
				goto out;
			}
