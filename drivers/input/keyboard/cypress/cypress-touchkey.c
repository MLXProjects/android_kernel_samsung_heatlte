...
#include "HeatLTE_F05M03_20140416.txt"
static u8 fw_ver_file = 0x05;
static u8 md_ver_file = 0x03;

u8 module_divider[] = {0, 0xff};
#include "cy8cmbr_swd.h"
#endif//TK_HAS_FIRMWARE_UPDATE
...
static int touchkey_keycode[] = { 0,
	...
	KEY_RECENT,  KEY_HOMEPAGE, KEY_BACK, KEY_SEARCH,

#elif defined(TK_USE_4KEY_TYPE_NA)
	KEY_SEARCH, KEY_BACK, KEY_HOMEPAGE, KEY_RECENT, 
		...
		u8 reg, u8 *val, unsigned int len)
{
	...
	while (retry--) {
		...
		if (err >= 0)
			return 0;
		dev_err(&client->dev, "%s: i2c transfer error, err=%d\n",
		       __func__, err);
		mdelay(10);
	}
	return err;
...
#endif//defined(TK_HAS_FIRMWARE_UPDATE)
...
int tkey_load_fw_built_in(struct touchkey_i2c *tkey_i2c)
{
	...
	tkey_i2c->update_info.fw_data = tkey_i2c->update_info.firm_data->data;
	tkey_i2c->update_info.fw_size = tkey_i2c->update_info.firm_data->size;
...
int tkey_load_fw_sdcard(struct touchkey_i2c *tkey_i2c)
{
	...
	int ret = 0;
/*	unsigned int nSize;*/
	u8 **ums_data = (u8**)&tkey_i2c->update_info.fw_data;
	...
	printk(KERN_NOTICE
		"touchkey:start, file path %s, size 0x%08x Bytes\n",
		TKEY_FW_PATH, (int)fsize);
	...
	*ums_data = kmalloc(fsize, GFP_KERNEL);
	...
	nread = vfs_read(fp, (char __user *)*ums_data,
		fsize, &fp->f_pos);
	printk(KERN_NOTICE "touchkey:nread 0x%08x Bytes\n", (int)nread);
	if (nread != fsize) {
		printk(KERN_ERR
...
#define BIN_HEADER_SIZE 0x10
	pr_debug("%s: bin header[0,1,0x%02x,0x%02x]="
		"%02x %02x %02x %02x\n",
			__func__, 
		BIN_HEADER_SIZE-2,
		BIN_HEADER_SIZE-1,
		tkey_i2c->update_info.fw_data[0], 
		tkey_i2c->update_info.fw_data[1], 
		tkey_i2c->update_info.fw_data[BIN_HEADER_SIZE-2],
		tkey_i2c->update_info.fw_data[BIN_HEADER_SIZE-1]);

	tkey_i2c->update_info.fw_data += BIN_HEADER_SIZE;
	tkey_i2c->update_info.fw_size = nread - BIN_HEADER_SIZE;

	pr_debug("%s: bin data[0,1,0x%04x,0x%04x]="
			"0x%02x 0x%02x 0x%02x 0x%02x\n",
			__func__, 
			tkey_i2c->update_info.fw_size - 2,
			tkey_i2c->update_info.fw_size - 1,
			tkey_i2c->update_info.fw_data[0], 
			tkey_i2c->update_info.fw_data[1],
			tkey_i2c->update_info.fw_data[tkey_i2c->update_info.fw_size - 2],
			tkey_i2c->update_info.fw_data[tkey_i2c->update_info.fw_size - 1]);
...
int tkey_load_fw_header(struct touchkey_i2c *tkey_i2c)
{
	tkey_i2c->update_info.fw_data = BinaryData;
	tkey_i2c->update_info.fw_size = sizeof(BinaryData);

	pr_debug("%s: firmware size=0x%04x\n", __func__, 
			tkey_i2c->update_info.fw_size);
	pr_debug("%s: firmware data[0~1]=0x%02x 0x%02x\n",
			__func__, 
			tkey_i2c->update_info.fw_data[0], tkey_i2c->update_info.fw_data[1]);
	pr_debug("%s: firmware data[0x00, 0x01, 0x%04x, 0x%04x]="
			"0x%02x 0x%02x 0x%02x 0x%02x\n",
			__func__,
			tkey_i2c->update_info.fw_size-2,
			tkey_i2c->update_info.fw_size-1,
			tkey_i2c->update_info.fw_data[0x00], 
			tkey_i2c->update_info.fw_data[0x01], 
			tkey_i2c->update_info.fw_data[tkey_i2c->update_info.fw_size-2], 
			tkey_i2c->update_info.fw_data[tkey_i2c->update_info.fw_size-1]);
	return 0;
}
...
int touchkey_load_fw(struct touchkey_i2c *tkey_i2c, u8 fw_path)
{
	...
	case FW_IN_SDCARD:
		...
		break;
	case FW_HEADER:
		ret = tkey_load_fw_header(tkey_i2c);
		break;
	default:
...
void touchkey_unload_fw(struct touchkey_i2c *tkey_i2c)
{
	...
	case FW_BUILT_IN:
		...
		break;
	case FW_IN_SDCARD:
		kfree(tkey_i2c->update_info.fw_data);
		tkey_i2c->update_info.fw_data = NULL;
		break;
	default:
		break;
	}
...
#ifdef  TK_HAS_FIRMWARE_UPDATE
#if 1
static void touchkey_i2c_update_work(struct touchkey_i2c *tkey_i2c)
{
#else
...
/*	const struct firmware *firm_data = NULL;*/
	...
	tkey_i2c->pdata->set_clkdata_gpio();
	
	while (retry--) {
		//ret = ISSP_main(tkey_i2c);
		ret = cy8cmbr_swd_program(tkey_i2c->dev, 
				tkey_i2c->pdata->power_on,
				tkey_i2c->pdata->gpio_sda,
				tkey_i2c->pdata->gpio_scl,
				tkey_i2c->update_info.fw_data,
				tkey_i2c->update_info.fw_size);
	...
	tkey_i2c->pdata->set_clkdata_i2c();
	msleep(20);
	ret = touchkey_i2c_check(tkey_i2c);
	if (ret < 0)
		;//goto err_fw_update;

	dev_info(&tkey_i2c->client->dev, "f/w ver=0x%02X, module ver=0x%02X\n",
...
int touchkey_fw_update(struct touchkey_i2c *tkey_i2c, u8 fw_path, bool bforced)
{
	...
	/* f/w info */
	dev_info(&tkey_i2c->client->dev, "fw ver ic 0x%02x, fw ver phone 0x%02x\n",
		tkey_i2c->fw_ver_ic, fw_ver_file);
	dev_info(&tkey_i2c->client->dev, "module ver ic 0x%02x, module ver phone 0x%02x\n",
		tkey_i2c->md_ver_ic, md_ver_file);

	if (unlikely(bforced)) {
		dev_info(&tkey_i2c->client->dev, "forced fw update\n");
		goto run_fw_update;
	}
	...
	/* pass update */
	dev_info(&tkey_i2c->client->dev, "fw update not required\n");
	touchkey_unload_fw(tkey_i2c);
	return 0;
 run_fw_update:
	dev_info(&tkey_i2c->client->dev, "fw update\n");
	//queue_work(tkey_i2c->fw_wq, &tkey_i2c->update_work);
	touchkey_i2c_update_work(tkey_i2c);
	return 0;
}
...
void touchkey_backlight_control(int on)
{
	int data;
	int ret;
	static const int ledCmd[] = {TK_CMD_LED_OFF, TK_CMD_LED_ON};

	if (wake_lock_active(&gtkey_i2c->fw_wakelock)) {
		printk(KERN_DEBUG"touchkey:%s, wakelock active\n", __func__);
	}

	data = ledCmd[on];

	ret = i2c_touchkey_write(gtkey_i2c->client, (u8 *) &data, 1);
	if (ret < 0) {
		dev_err(&gtkey_i2c->client->dev, "%s: Error turn on led %d\n",
			__func__, ret);
...
EXPORT_SYMBOL(touchkey_backlight_control);
...
static int sec_touchkey_early_suspend(struct early_suspend *h)
{
	...
	gpio_pull_down_port(TOUCH_1V8_EN);
	dev_dbg(&tkey_i2c->client->dev, "%s\n", __func__);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(&tkey_i2c->client->dev, "called %s data[10] =%d,data[11] = %d\n", __func__,
	       data[10], data[11]);
	recent_sensitivity = ((0x00FF & data[10]) << 8) | data[11];
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(&tkey_i2c->client->dev, "called %s data[2] = %d, data[3] =%d\n", __func__,
			data[2], data[3]);
	back_sensitivity = ((0x00FF & data[2]) << 8) | data[3];
...
#ifdef LED_LDO_WITH_REGULATOR
static DEVICE_ATTR(touchkey_brightness, S_IWUSR | S_IWGRP, NULL, brightness_control);
#endif
...
static int touchkey_power_on(bool on)
{
	...
	if(on){
		...
		printk("%s - 3TOUCH_1V8:ON\n",__func__);
		if(system_rev>=3)	
		gpio_set_value(TOUCH_1V8_EN,TOUCH_1V8_ON);
		
	}
	else{
		printk("%s - 3TOUCH_1V8:OFF\n",__func__);
		if(system_rev>=3)
	...
	const struct i2c_device_id *id)
{
	...
	tkey_i2c->pdata->init_platform_hw();	
	tkey_i2c->pdata->power_on(1);
	msleep(50);
	/* enable ldo11 */
    tkey_i2c->pdata->led_power_on(1);
...
#if defined(TK_HAS_FIRMWARE_UPDATE)
	ret = touchkey_fw_update(tkey_i2c, FW_HEADER, fw_download);
		if (ret < 0) {
...
/*	touchkey_init_fw_name(tkey_i2c);
	ret = touchkey_fw_update(tkey_i2c, FW_BUILT_IN, fw_download);
	if (ret < 0) {
		dev_err(&client->dev, "fw update fail\n");
		goto err_firmware_update;
	}*/
