...
#define TOUCH_BOOSTER_SHMOBILE  1

#if TOUCH_BOOSTER_SHMOBILE
#include <mach/pm.h> 
#endif
...
#if TOUCH_BOOSTER_SHMOBILE
static struct delayed_work delayed_cpufreq_work;
#endif
...
static void setting_cpufreq(uint32_t on)
{
	if ( 0 == on ){
		if (!dfs_started)
			schedule_delayed_work(&delayed_cpufreq_work, msecs_to_jiffies(1000));
	}else if ( 1 == on ){
		cancel_delayed_work_sync(&delayed_cpufreq_work);
		if (dfs_started) {
			int ret = 0;
			ret = stop_cpufreq();
				   ...
				   const struct i2c_device_id * id)
{
	...
	INIT_DELAYED_WORK(&delayed_cpufreq_work, delayed_start_cpufreq);
#endif
