...
static void ts_power_enable(int en)
{
	...
	touch_regulator_1v8 = regulator_get(NULL,"sensor_led_3v");
...
#endif
	touch_regulator_3v3 = regulator_get(NULL,"vtsp_3v");
	...
	else
	{
		...
		if (ret) {
			tsp_info(KERN_ERR "%s: touch_regulator_3v3 disable failed (%d)\n",__func__, ret);
		}
