...
static u8 usb_phy_read(u64 reg)
{
	...
	pr_info("Value of PHY_READ is %x\n", monreg);
	return monreg;
}
...
static int usb_core_clk_ctrl(struct r8a66597 *r8a66597, bool clk_enable)
{
	   ...
	   if (clk_enable) {
		if (!usb_clk_status) {
			...
			pr_info("%s: SYSC_PSTR: %08x\n",
					__func__, __raw_readl(PSTR));
			pr_info("%s: SYSC_MSTPSR3: %08x\n",
					__func__, __raw_readl(MSTPSR3));
			pr_info("%s: SYSC_MMSTPCR2: %08x\n",
				   ...
				   struct r8a66597_request *req)
{
	...
	/* Allocate DMA only to Bulk with size larger than 1023 */
	if (req->req.length < 1024)
		 return -EINVAL;
