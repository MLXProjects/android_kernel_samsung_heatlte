...
static phys_addr_t __init u2mem_reserve(phys_addr_t base, unsigned int size,
		phys_addr_t align)
{
	/* If the kernel does NOT see the absolute start of SDRAM
	 * we must revert to old hard-coded frame buffer size.
	 * This is run-time compliant with 14w12 memmap.
	 */
	if (SDRAM_KERNEL_START_ADDR > SDRAM_START_ADDR) {
		pr_alert("Revert frame buffer to hard-coded address 0x%x.\n",
			SDRAM_FRAME_BUFFER_START_ADDR);
		base = SDRAM_FRAME_BUFFER_START_ADDR;
		return base;
	}
