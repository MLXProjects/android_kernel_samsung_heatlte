...
#if CONFIG_SH_RAMDUMP_DEBUG_ANDROID_BOOT_FREEZE > 0
/* this is for android boot stuck debug feature */
static int android_booted;
module_param_named(android_booted, android_booted, int, S_IRUGO | S_IWUSR | S_IWGRP);
static void android_booted_callback(unsigned long ignored)
{
	if (!android_booted) {
		printk("ANDROID BOOT HAS NOT COMPLETED IN %ds -> PANIC!",
				CONFIG_SH_RAMDUMP_DEBUG_ANDROID_BOOT_FREEZE);
		BUG();
	}
}
static DEFINE_TIMER(android_booted_timer, android_booted_callback, 0, 0);
#endif
...
/* Split name length fixed to 20 chars to make parsing easier */
#define MAX_NAME_LENGTH 20
struct split_resource {
	phys_addr_t start;
	phys_addr_t end;
	char name[MAX_NAME_LENGTH];
} __packed;

#define MAX_NUMBER_OF_SPLITS 20
static struct split_resource split_resources[MAX_NUMBER_OF_SPLITS];
static atomic_t split_idx = ATOMIC_INIT(-1);
void register_ramdump_split(const char *name, phys_addr_t start,
		phys_addr_t end)
{
	unsigned int i;
	...
	if (start < end &&
		((start >= SDRAM_START_ADDR && end < memblock_end_of_DRAM()) ||
		(start >= ICRAM0_START_ADDR && end <= ICRAM0_END_ADDR) ||
		(start >= ICRAM1_START_ADDR && end <= ICRAM1_END_ADDR) ||
		(start >= SECURE_RAM_START_ADDR &&
		end <= SECURE_RAM_END_ADDR))) {
		i = atomic_inc_return(&split_idx);
		if (i == MAX_NUMBER_OF_SPLITS) {
			pr_err("ERROR: splits full. Ignoring %s 0x%x-0x%x\n",
					name, start, end);
			return;
		}
		/* this API is meant to be used during early boot
		 * so if someone tries to register too long name
		 * then better to stop it now */
		BUG_ON(strlen(name) > MAX_NAME_LENGTH);
		dprintk("%s adding split %s 0x%x-0x%x to %d slot\n", __func__,
				name, start, end, i);
		strncpy(&split_resources[i].name[0], name, MAX_NAME_LENGTH);
		split_resources[i].start = start;
		split_resources[i].end = end;
	} else {
		pr_err("ERROR: Invalid RAMDUMP split! %s 0x%x-0x%x\n",
				name, start, end);
		}
}
...
static resource_size_t ramdump_remap_resources(struct ramdump_plat_data *pdata)
{
	...
	for (i = 0; i < pdata->num_resources; i++) {
		...
		if (!v_start) {
			pr_err("%s ioremapping %x-%x failed!\n", __func__,
					hwr->start, h_end);
...
static int __init ramdump_probe(struct platform_device *pd)
{
	...
	phys_addr_t reg_phys_start, reg_phys_end;
	...
	mm = (void __force *) ramdump->reg_dump_base + ramdump->reg_dump_size -
			sizeof(unsigned int);
	*((unsigned int *)mm) = (unsigned int)__pv_phys_offset;
	mm = mm - strlen(magic_marker);
	strcpy(mm, magic_marker);
	mm = mm - 4;
	*((unsigned int *)mm) = virt_to_phys(split_resources);
	...
	ramdump->lsi_reg_dump_base = ramdump->core_reg_dump_base +
			ramdump->core_reg_dump_size * num_possible_cpus();
	...
	/* Register the area that contains the LSI register dump for easy
	 * parsing from ramdump. Needs to be physical address. */
	reg_phys_start = ramdump->reg_dump_base_phys +
			ramdump->core_reg_dump_size * num_possible_cpus();
	reg_phys_end = reg_phys_start + hw_reg_dump_size - 1;
	register_ramdump_split("hwregisters", reg_phys_start, reg_phys_end);
	...
	VMCOREINFO_NUMBER(__pv_phys_offset);
	VMCOREINFO_LENGTH(virt_offset, PAGE_OFFSET);

#if CONFIG_SH_RAMDUMP_DEBUG_ANDROID_BOOT_FREEZE > 0
	mod_timer(&android_booted_timer,
			jiffies + CONFIG_SH_RAMDUMP_DEBUG_ANDROID_BOOT_FREEZE*HZ);
#endif
	return 0;
...
static int __init init_ramdump(void)
{
	...
	ret = platform_driver_probe(&ramdump_pdriver, ramdump_probe);
	if (ret) {
