...
static int unused_gpios_vivalto_rev0[] = {
	GPIO_PORT3, GPIO_PORT4, GPIO_PORT21, GPIO_PORT24,
	GPIO_PORT25,GPIO_PORT26, GPIO_PORT27, GPIO_PORT29,
	GPIO_PORT36, GPIO_PORT44, GPIO_PORT46, GPIO_PORT86,
	GPIO_PORT87, GPIO_PORT104, GPIO_PORT108, GPIO_PORT140,
...
#if defined (CONFIG_BACKLIGHT_KTD2801)
...
#define TSP_INT_GPIO_PIN   	(32)

static struct tsp_dev_info ist30xxb_platformdata = {
	.gpio = TSP_INT_GPIO_PIN,
};
static struct i2c_board_info __initdata touch_i2c_devices[] = {
	  {
		I2C_BOARD_INFO("IST30XX", 0x50),
             .platform_data = (void *)&ist30xxb_platformdata,
	  },
};
#endif
...
static struct pinctrl_map vivaltolte_pinctrl_map[] __initdata = {
...
static void __init board_init(void)
{
	...
	sh_pfc_register_mappings(vivaltolte_pinctrl_map,
				 ARRAY_SIZE(vivaltolte_pinctrl_map));
	r8a7373_pinmux_init();
	...
	/* VivaltoLTE Rev_0_0 and Rev_0_1 has same set of unsued GPIOs */
	for (inx = 0; inx < ARRAY_SIZE(unused_gpios_vivalto_rev0); inx++)
		unused_gpio_port_init(unused_gpios_vivalto_rev0[inx]);
	...
	/* In vivalto HW for sdhi0 data lines internal pullup
	* needs to be enabled */
...
if (system_rev < 2) // From HW Rev.02
	 add_primary_cam_flash_rt8547(GPIO_PORT99, GPIO_PORT100);
else  
	   add_primary_cam_flash_ktd2692(GPIO_PORT99, GPIO_PORT100);

	add_isx012_primary_camera();
...
/* GPS Init */
	...
	gps_gpio_init();
#endif
	r8a7373_set_panel_func(r_mobile_hx8369b_panel_func);
...
static const char *vivalto_compat_dt[] __initdata = {
	"renesas,vivaltolte",
	NULL,
};

DT_MACHINE_START(VIVALTOLTE, "vivaltolte")
