			...
			struct sh_plat_hp_data *hpdata)
{
	...
	for_each_possible_cpu(cpu) {
		if (cpu == 0)
			continue;
		BUG_ON(tl[cpu].thresh_plugout > cur_policy->cpuinfo.max_freq ||
			tl[cpu].thresh_plugout < cur_policy->cpuinfo.min_freq ||
			tl[cpu].thresh_plugin > cur_policy->cpuinfo.max_freq ||
			tl[cpu].thresh_plugin < cur_policy->cpuinfo.min_freq);
	}
