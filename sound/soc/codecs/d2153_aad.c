...
void d2153_set_headset_polarity(bool value)
{
	gheadset_polarity = value;
	dlg_info("%s() gheadset_polarity = %d\n", __func__, gheadset_polarity);
}
EXPORT_SYMBOL(d2153_set_headset_polarity);

int d2153_get_headset_polarity()
{
	dlg_info("%s() gheadset_polarity = %d\n", __func__, gheadset_polarity);
	return gheadset_polarity;
}
EXPORT_SYMBOL(d2153_get_headset_polarity);
...
int d2153_codec_power(struct snd_soc_codec *codec, int on)
{
	...
	if (on == 1 && d2153_codec->power_mode == 0) {
		...
		/* In Chip errata versions 0xAB and below, dont turn off
		the Audio regulator */
		if (D2153_AB_SILICON < d2153_aad->chip_rev) {
			...
			if (regval != 0x00) {
				dlg_err("%s() LDO_AUD1 off regval[0x%x]\n",
					__func__, regval);
				d2153_reg_write(d2153_codec->d2153_pmic,
					0x5E, 0x00);
			}
#endif
			d2153_put_i2c_hwsem();
...
static void d2153_aad_gpio_monitor_timer_work(struct work_struct *work)
{
	...
	/*Headset polarity check */
	if (gheadset_polarity)
		state_gpio = !state_gpio;
					 ...
					 const struct i2c_device_id *id)
{
	...
	d2153_aad->button_detect_rate = 60;
