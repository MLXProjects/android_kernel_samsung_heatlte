...
static int fsi_dma_exit(struct fsi_priv *fsi, int is_play)
{
	...
	if (io == NULL) {
		sndp_log_err("fsi_dma_exit:Stream Io Pntr NULL. Exiting.\n");
		return 0;
	}

	if (!io->dma_chan) {
		sndp_log_err("fsi_dma_exit:DMA_Chan Member NULL. Exiting.\n");
		return 0;
	}
	...
	if (io->substream) {
		...
		if (dai == NULL) {
			sndp_log_err("fsi_dma_exit:DAI Pntr NULL. exiting..\n");
			return 0;
		}
	...
	struct snd_soc_dai *dai)
{
	...
	/* Check for fsi pointer and return -EFAULT if it is NULL */
	if (fsi == NULL) {
		sndp_log_err("fsi_dai_trigger_in_fm: FSI Pntr NULL.Exiting..\n");
		return -EFAULT;
	}
...
static int fsi_of_parse(char *name, struct device_node *np,
		struct sh_fsi_platform_info *info, struct device *dev)
{
	...
	struct {
		...
		{ "master-bit-clock-inverted", SH_FSI_BRM_INV },
		{ "slave-bit-clock-inverted", SH_FSI_BRS_INV },
		{ "master-lr-clock-inverted", SH_FSI_LRM_INV },
		{ "master_bit-clock-inverted", SH_FSI_LRS_INV },
		{ "i2s", SH_FSI_FMT_I2S },
		{ "mono", SH_FSI_FMT_MONO },
		{ "mono-delay", SH_FSI_FMT_MONO_DELAY },
		{ "pcm", SH_FSI_FMT_PCM },
		{ "tdm", SH_FSI_FMT_TDM },
		{ "tdm-delay", SH_FSI_FMT_TDM_DELAY },
		{ "fsi_out_slavemode", SH_FSI_OUT_SLAVE_MODE },
		{ "fsi_in_slavemode", SH_FSI_IN_SLAVE_MODE },
	...
	for (j = 0; j < num_strings; j++) {
		...
		ret = of_property_read_string_index(np, name, j, &prop);
		if (ret) {
			dev_err(dev, "%s invalid arguments\n", __func__);
			return -EINVAL;
		}
		...
		for (i = 0; i < ARRAY_SIZE(of_parse_property); i++) {
			if (strcmp(prop, of_parse_property[i].name) == 0) {
	...
	info->port_flags = flags;

	return 0;

}

static struct sh_fsi_platform_info info[2];
...
static struct of_device_id fsi_of_match[] = {
	{ .compatible = "renesas,sh_fsi", &fsi_regs },
	{ .compatible = "renesas,sh_fsi2", &fsi2_regs },
	{},
};
...
{
	struct fsi_master *master;
	const struct platform_device_id	*id_entry = NULL;
	const struct of_device_id *of_id = NULL;
	struct device_node *np = pdev->dev.of_node;
	const struct fsi_core *core = NULL;
	struct resource *res;
	unsigned int irq;
	int ret = 0;
	...
	if (np) {
		...
		if (of_id) {
			core = (struct fsi_core *)of_id->data;
			ret = of_property_read_u32_index(np, "id", 0,
							 &pdev->id);
			if (ret < 0) {
				dev_err(&pdev->dev, "could not get device id\n");
				return -EINVAL;
			}
			if (of_find_property(np, "always_slave", NULL))
				info[pdev->id].always_slave = 1;
			ret = fsi_of_parse(pdev->id ? "fsib" : "fsia", np,
						&info[pdev->id], &pdev->dev);
			if (ret) {
				dev_err(&pdev->dev, "parsing invalid:string not found\n");
				return -EINVAL;
			}
			pdev->dev.platform_data = &info[pdev->id];
		}
	} else {
	...
	master->regs		= (struct fsi_regs *)core;
#ifdef USE_DMA
