...
int  g_sndp_voicecall_debug_level;
/* for Wake Lock */
...
static const struct file_operations voicecall_debug_fops = {
	.read           = sndp_proc_voicecall_debug_read,
	.write          = sndp_proc_voicecall_debug_write,
	.llseek         = default_llseek,
};
	...
	struct snd_soc_card *fsi_soc_card)
{
	...
	struct proc_dir_entry	*voicecall_debug_entry = NULL;

	sndp_log_debug_func("start\n");

	g_pt_start = SNDP_PT_NOT_STARTED;
	g_sndp_voicecall_debug_level = SNDP_VOICECALL_DEBUG_OFF;
	...
	if (NULL != g_sndp_parent) {
		...
		/* create file for voice call debug level entry */
		voicecall_debug_entry = proc_create_data(VOICECALL_DEBUG_LEVEL,
					S_IRUGO | S_IWUGO,
					g_sndp_parent, &voicecall_debug_fops, NULL);
	...
	//g_sndp_log_level = LOG_FUNC_PRINT;
	...
	/* FSI master */
	iRet = common_set_fsi2cr(SNDP_NO_DEVICE, STAT_ON);
	if (ERROR_NONE != iRet)
		goto null_ptr_err;
...
void sndp_exit(void)
{
	/* Proc entry remove */
	if (NULL != g_sndp_parent)
		remove_proc_subtree(SNDP_DRV_NAME, NULL);
...
static int sndp_proc_voicecall_debug_read(struct file *file, char __user *buf,
 ...
 */
static int sndp_proc_voicecall_debug_write(
	struct file *filp,
	...
	loff_t *ppos)
{
	...
	g_sndp_voicecall_debug_level = (u_int)uiIn & VOICECALL_DEBUG_LEVEL_MAX;
	...
	struct snd_soc_dai *dai)
{
	...
	case SNDP_MODE_INCALL:
		/* Call process route */
		sndp_log_debug_func("Voice call debug level is %d\n",
					g_sndp_voicecall_debug_level);
		if((g_sndp_voicecall_debug_level >  SNDP_VOICECALL_DEBUG_OFF) &&
				(SNDP_PCM_IN == substream->stream)) {
			sndp_log_debug_func("Reuse FM capture for voice\
					call debugging\n");
	...
	default:
		if (SNDP_FM_RADIO_RX &
			...
			sndp_log_debug_func("default mode\n");
...
static void sndp_work_voice_start(struct sndp_work_info *work)
{
	...
	if (!(0 == iRet || 1 == iRet)) {  /* 0:success 1:active */
		sndp_log_err("%s: Unable to get Power-domain Status\n",
			     __func__);
		/* Revert the status */
...
static void sndp_work_voice_stop(struct sndp_work_info *work)
{
    ...
    sndp_log_info("sndp_work_voice_stop: Waiting for g_watch_stop_clk for %d msecs..\n",
            SNDP_WATCH_CLK_TIME_OUT);
	...
	if (ERROR_NONE != iRet)
		sndp_log_err("%s: Unable to update Power-domain Status.\n",
			     __func__);
...
#else /* !__SNDP_INCALL_CLKGEN_MASTER */
	/* FSI master process */
	iRet = common_set_pll22(work->old_value,
			STAT_OFF,
			g_bluetooth_band_frequency);
	if (ERROR_NONE != iRet)
		return;
    ...
    sndp_log_info("sndp_work_voice_stop-->end\n");
...
#ifndef __SNDP_INCALL_CLKGEN_MASTER
	/* CLKGEN master setting */
	iRet = common_set_pll22(old_value, STAT_OFF, g_bluetooth_band_frequency);
	 if (ERROR_NONE != iRet)
		return iRet;
	iRet = common_set_fsi2cr(SNDP_NO_DEVICE, STAT_OFF);
	if (ERROR_NONE != iRet)
...
#ifndef __SNDP_INCALL_CLKGEN_MASTER
	/* FSI master setting */
	iRet = common_set_pll22(new_value,
			STAT_ON,
			g_bluetooth_band_frequency);
	if (ERROR_NONE != iRet)
		return iRet;
...
#ifdef __SNDP_INCALL_CLKGEN_MASTER
	/* CLKGEN master setting */
	ret = common_set_fsi2cr(SNDP_NO_DEVICE, STAT_OFF);
	if (ERROR_NONE != ret)
		return;
#else /* !__SNDP_INCALL_CLKGEN_MASTER */
	if (!(SNDP_BLUETOOTHSCO & SNDP_GET_DEVICE_VAL(new_value)))
		/* FSI master setting */
		ret = common_set_pll22(new_value,
				STAT_ON,
				g_bluetooth_band_frequency);
		if (ERROR_NONE != ret)
			return;
...
/*	g_sndp_start_call_wait = 0;*/
	...
	if (SNDP_MODE_INCALL != SNDP_GET_MODE_VAL(GET_OLD_VALUE(SNDP_PCM_OUT))) {
		...
		ret = common_set_fsi2cr(SNDP_NO_DEVICE, STAT_ON);
		if (ERROR_NONE != ret)
			return;
#else /* !__SNDP_INCALL_CLKGEN_MASTER */
		if (!(SNDP_BLUETOOTHSCO & SNDP_GET_DEVICE_VAL(old_value)))
			/* FSI master process */
			ret = common_set_pll22(old_value,
					STAT_OFF,
					g_bluetooth_band_frequency);
			if (ERROR_NONE != ret)
				return;
...
static void sndp_watch_stop_clk_cb(void)
{
    ...
    sndp_log_err("sndp_watch_stop_clk_cb->Start");
    ...
    sndp_log_err("sndp_watch_stop_clk_cb->End");
...
static void sndp_work_start(const int direction)
{
	...
	if (SNDP_MODE_INCALL == SNDP_GET_MODE_VAL(uiValue)) {
		...
		if (SNDP_IS_FSI_MASTER_DEVICE(dev)) {
			...
			fsi_set_slave(true);
			iRet = common_set_fsi2cr(dev, STAT_OFF);
			if (ERROR_NONE != iRet)
				return;
		}
	}
	...
	if (!((E_PLAY | E_CAP) & g_sndp_playrec_flg)) {
		...
		if (SNDP_MODE_INCALL == SNDP_GET_MODE_VAL(uiValue)) {
			...
			if (SNDP_IS_FSI_MASTER_DEVICE(dev)) {
				...
				fsi_set_slave(false);
				ret = common_set_fsi2cr(dev, STAT_ON);
				if (ERROR_NONE != ret)
					return;
			}
		}
...
#else /* !__SNDP_INCALL_CLKGEN_MASTER */
		/* FSI master setting */
		iRet = common_set_pll22(SNDP_PLAYBACK_EARPIECE_INCALL,
				 STAT_ON,
				 g_bluetooth_band_frequency);
		if (ERROR_NONE != iRet)
			return iRet;
